...
import {
  timeout,
  createRandomPointOnCircumference,
  asyncForEach,
...
export class BSTLevelHandler {
    ...
    this.levels = [
      ...
      new Level(
        ...
        async function () {
          ...
          logo
            .attr("xlink:href", "res/images/logo.svg")
          ...
          const clearMainMenu = () => {
            temp.main_menu_circle
              ...
              .duration(1000)
              .style("opacity", 0);
            dataHandler.logo.transition().duration(1000).style("opacity", 0);
            setTimeout(() => {
              logo.remove();
              temp.main_menu_circle.remove();
            }, 1000);
          };
          const storyModeButton = new cButton(
            "Storymode",
            w / 3,
            h * 0.66,
            svg,
          ...
          const SandboxButton = new cButton(
            "Sandbox",
            (w * 2) / 3,
            h * 0.66,
            svg,
            () => {
              clearMainMenu();
              temp.goToLevel(temp.levels[temp.levels.length - 1]);
            }
          );
        ...
        async function () {
          await albot.say(
            "A circle reveals its value when you click on it.",
            0,
            0
          );
        }
      ...
      new Level(
        ...
        async function () {
          ...
          circleManager.generateNodes(
            5,
            true,
            false,
            temp.isLevelComplete.bind(temp)
          );
          repaint();
          await albot.say(
            [
      ...
      new Level(
        ...
        async function () {
          await albot.say(
            [
              "Each node has a key and a value. You can look up the value by clicking on the node.",
            ],
            40,
            40
          );
      ...
      new Level(
        ...
        async function () {
          await albot.say(
            "There must be a way to organize the nodes that allows us to find values faster.",
            0,
            0
          );
        }
      ...
      new Level(
        ...
        "Find",
        async function () {
          await albot.say(["Let us arrange the nodes in an array!"], 40, 40);

          if (!temp.skip_introduction) await timeout(1000);
          var circles = circleManager.generateNodes(
            5,
            true,
            false,
            temp.isLevelComplete.bind(temp)
      ...
      new Level(
        ...
        async function () {
          await albot.say(
            [
              "This is similar to before, when the circles were not organized at all...",
              "I think we should organize the nodes INSIDE the array.",
            ],
            0,
            0
          );
      ...
      new Level(
        ...
        async function () {
          await albot.say(
            ["Now the array is sorted. Maybe this helps?"],
            40,
            40
          );
          if (!temp.skip_introduction) await timeout(1000);
        ...
        async function () {
          await albot.say(
            [
              "The fastest reliable way to do this, is with binary search",
              "To find X, do this:",
              "First check the middle elements value",
              "If X is smaller, then go to the middle of the LEFT SIDE",
              "If X is larger, then go to the middle of the RIGHT SIDE",
              "Do this, untill you hit the element. If you can't go further, then the element is not in the array",
            ],
            0,
            0
          );
      ...
      new Level(
        ...
        async function () {
          await albot.say(
            [
              "Could this be the perfect way of storing nodes?",
      ...
      new Level(
        ...
        async function () {
          ...
          await albot.say(
            [
              "Ah, yes...",
              "With the sorted array, I can find elements very fast. But INSERTING new elements can be very slow!",
              "Look at this:",
            ],
            40,
            40
          ...
          var input_circles = circleManager.generateNodes(
            5,
            true,
            false,
            temp.isLevelComplete.bind(temp)
          ...
          var array = dataHandler.makeArray(
            circles.length + input_circles.length,
            "Sorted array"
          );
          array.static = true;
          array.addDataToArray(circles);
      ...
      new Level(
        ...
        async function () {
          await albot.say(
            [
              ...
              "It is similar to a sorted array: Going left means going to smaller values. Going right means going to larger values.",
              "When accessing nodes, you have to start from the root. Nodes only know their children and their parent",
              "Right-click the root to test the functionalities!",
            ],
            40,
            40
          );
          ...
          albot.say(
            [
              'Right-click on the root to experiment with inserting, deleting, and finding values.",  "Click on a node to complete the level',
            ],
            0,
            0
          );
        ...
        async function () {
          await albot.say(
            "If you right-click the root of the tree, you can experiment with inserting, deleting, and finding values.",
            0,
            0
          );
        }
      ...
      new Level(
        ...
        async function () {
          await albot.say(
            [
              "When searching for values, you always start at the root.",
              "Then you compare the value of the current node with value you search for.",
              "If the value at the current node is larger, then go left, else go right!",
            ],
            40,
            40
          );
          ...
          var queue = new CircleQueue(circles, bestRoot);
          temp.successCircle = circleManager.circles.filter(
            (d) => bst.root !== d
          )[Math.floor(Math.random() * (circleManager.circles.length - 1))];
          ...
          var closure = async () => {
            ...
            albot.say(
              [
                "If you are unsure how to do this, press ESC and try the BST sandbox.",
                "Try to find " + temp.successCircle.value,
              ],
              0,
              0
            );
        ...
        async function () {
          await albot.say(
            ["Nice job, my humanoid friend!", "Let us try with a larger tree"],
            0,
            0
          );
        }
      ...
      new Level(
        ...
        async function () {
          await albot.say(
            [
              "Wow, you are getting good at this!, 'Some day you humans are going to catch up to us computers!",
            ],
            0,
            0
          );
      ...
      new Level(
        ...
        async function () {
          await albot.say(
            "Let us find out how to insert nodes in a Binary Search Tree.",
            0,
            0
          );
        }
      ...
      new Level(
        ...
        async function () {
          await albot.say(
            ["We have to figure out the correct location for the new node."],
            40,
            40
          );
          ...
          albot.say(
            [
              "If you are unsure how to do this, press ESC and try the BST sandbox.",
              "Try to insert " + temp.successCircle.value,
            ],
            0,
            0
          );
        ...
        async function () {
          await albot.say(
            "When inserting nodes, you have to find the correct place, and when you hit a NULL pointer, then insert the node there.",
            0,
            0
          );
        }
      ...
      new Level(
        ...
        async function () {
          ...
          await albot.say(
            [
              "To delete a node, you first have to find it.",
              "Then find the smallest value in its right subtree. That node will replace the node to be deleted.",
              "To remove a subtree, click and drag over links to cut them.",
              "You can add a node to the tree by dragging and dropping it on null pointers",
            ],
            40,
            40,
            albot_speed
          ...
          albot.say(
            [
              "If you are unsure how to do this, press ESC and try the BST sandbox.",
              "Try to delete " + temp.successCircle.value,
            ],
            0,
            0
          );
        ...
        (n) => {
          ...
          return (
            temp.bst.nodeToInsert.locked_to_tree !== temp.bst &&
            !d3
              .selectAll(".circle")
              .data()
              .filter((d) => {
        ...
        async function () {
          albot.say(
            [
              "Nice! Now we have all the functions needed for a BST!",
              "This is awesome!",
            ],
            0,
            0
        ...
        async function () {
          await albot.say(
            [
              "When deleting, navigate to the node you want to delete (X).",
              "Find X's smallest value in it's RIGHT subtree. If the right subtree is null, then use the left child instead",
              "That will change places with X and get its left and right child",
              "Then delete X from the tree.",
            ],
            0,
            0
          );
      ...
      new Level(
        ...
        async function () {
          ...
          await albot.say(
            [
              "I tried writing some code for inserting nodes into BSTs, but it seems I made a mistake!",
              "The highlighted line seems fishy.",
        ...
        (n) => {
          return (
            temp.bst &&
            temp.bst.isValid() &&
            circleManager.circles.every((c) => c.locked_to_tree === temp.bst)
          );
        },
        // on Success
        async function () {
          await albot.say(
            ["Yes, you did it!", "Now it looks like a proper BST"],
            0,
            0
          );
        },
        ...
        async function () {
          await albot.say(
            [
              "Each node has two properties, left and right. They are pointers to other nodes or NULL",
              " ",
              "Left should point to a node with a smaller value, and right should point to a node with a larger value",
              " ",
              'In the code, the variable "node" is the new node, and "h" is the navigation node.',
        ...
        async function () {
          var code = [
            ...
            {
              text: "        h.right = insert(node, h.right);",
              editable: false,
            },
            { text: "    }", editable: false },
      ...
      new Level(
        ...
        async function () {
          ...
          await albot.say(
            [
              "Looks like I made an error at a different place now!",
              "Edit the highlighted lines and press the button to rebuild the tree.",
            ],
            40,
            40,
            albot_speed
        ...
        async function () {
          await albot.say(
            [
              "Uuh! You are on a roll!",
              "The world will bow down to human and computer, best friends forever!",
            ],
            0,
            0
          );
        ...
        async function () {
          await albot.say(
            [
              ...
              " ",
              'In the code, the variable "node" is the new node, and "h" is the navigation node.',
              "Check if the proper children are used for navigating the tree",
            ],
            0,
            0
          );
      ...
      new Level(
        ...
        async function () {
          ...
          await albot.say(
            [
              "...",
              "Okay, wow. I accidentally deleted all my insert code...",
              "Can you help me and write new code?",
            ],
            40,
            40,
            albot_speed
          ...
          albot.say(
            [
              ...
              "If you need inspiration, you can revisit the two previous levels.",
              "Try to write code for inserting nodes into a BST.",
            ],
            0,
            0
          );
        ...
        async function () {
          await albot.say(
            [
              "When inserting, you either:",
              "go left",
              "go right",
              "hit the existing value",
              "hit a NULL pointer and place the node there.",
              "Make sure the code handles all those scenarios",
              " ",
              "All the code for this is in the two previous levels.",
            ],
            0,
            0
          );
      ...
      new Level(
        ...
        async function () {
          await albot.say(
            [
              "Oh my processor...",
              "Looks like even BSTs can't solve all our problems",
              "Check out what happens for sorted inputs.",
              "This BST is unbalanced, so it performs badly.",
            ],
            40,
            40
          );
          ...
          albot.say(
            [
              "Look! The tree is very tall.",
              "Finding 10 will go through ALL the other nodes",
            ],
            0,
            0
          );
        ...
        async function () {
          await albot.say(
            "Can we make sure that the BST has good performance, even for sorted inputs?",
            0,
            0
          );
        }
        ...
        async function () {
          await albot.say(
            [
              ...
              "We use a Red Black Binary Search Tree. This will balance the tree!",
            ],
            40,
            40
          );
          ...
          var circles2 = circleManager.generateNodes(
            10,
            true,
            false,
            temp.isLevelComplete.bind(temp)
          ...
          await albot.say(
            ["Look how a big a difference it makes on the same input."],
            0,
            0
          );
      ...
      new Level(
        ...
        async function () {
          ...
          await albot.say(
            [
              "There are 3 different scenarios for balance rules:",
              "Scenario 1: Inserting a larger value in a subtree:",
              "If a node got two red children, then change their color to black",
            ],
            0,
            0
          );
          var text = new cText(
            "1st scenario: Inserting a larger value in a subtree",
            0,
            -150,
            40,
            1,
          ...
          repaint();
          var text = new cText(
            "2nd scenario: Inserting a smaller value in a subtree",
            0,
            400,
            40,
            1,
          ...
          await albot.say(
            [
              "Scenario 2: Inserting a smaller value in a subtree:",
              "If a node has two red lines going left, then rotate right, and balance afterwards",
            ],
            0,
            0
          );
          ...
          bst.updateLinks();
          var text = new cText(
            "3rd scenario: Inserting a value in betweeen a subtree",
            0,
            900,
            40,
            1,
          ...
          await albot.say(
            [
              "Scenario 3: Inserting a value in between a subtree:",
              "If a node has two red lines going left, then rotate right, and balance afterwards",
            ],
            0,
            0
          ...
          await albot.say(
            [
              "Now you see that all the BSTs are identical, even though the input order was different each time!",
              "When it works with the smaller ones, it also works with the larger ones!",
            ],
            0,
            0
          );
      ...
      new Level(
        ...
        async function () {
          await albot.say(
            [
              'Finding nodes in a RB-BST works just like in a regular BST!", "Left means smaller values, right means bigger values',
            ],
            40,
            40
          );
      ...
      new Level(
        ...
        async function () {
          await albot.say(
            [
              "Congratulations! You have cleared the story mode!",
              "I hope you learned something useful!",
              "You can revisit all levels by clicking ESC, or try out the sandbox mode!",
            ],
            0,
            0
          );
      ...
      new Level(
        ...
        async function () {
          ...
          const circleButton = new cButton(
            "Circle",
            w * 0.4,
            h * 0.9,
            svg,
          ...
          const arrayButton = new cButton(
            "Array",
            w * 0.6,
            h * 0.9,
            svg,
            () => {
              var arr = dataHandler.makeArray(10, "Array");
              dataHandler.addFigure(arr);
              repaint();
            }
          );
          dataHandler.addFigure(arrayButton);

          const BSTButton = new cButton("BST", w * 0.8, h * 0.9, svg, () => {
            var bst = dataHandler.makeBST(null);
            dataHandler.addFigure(bst);
            repaint();
          });
          dataHandler.addFigure(BSTButton);

          const RBBSTButton = new cButton("RB-BST", 80, 85, svg, () => {
            var bst = dataHandler.makeRedBlackBST(null);
            dataHandler.addFigure(bst);
            repaint();
          });
          dataHandler.addFigure(RBBSTButton);

          const ClearButton = new cButton("Clear screen", 80, 85, svg, () => {
          ...
          await albot.say(
            [
              "This is the sandbox level. Here you can do whatever you want and explore the functionality of the program.",
  ...
  async createLevel() {
    ...
    if (this.levels.indexOf(this.currentLevel) > 9)
      window.scissor.enabled = true;
    window.albot.ignoreSayCalls = false;
    this.svg
      .transition()
      .duration(500)
  ...
  async getReadyForNextLevel(n) {
    ...
    var text = new cText(
      ...
      window.windowSize.width / 2,
      window.windowSize.height * 0.95,
      30,
      0.5
    );
  ...
  createSuccessAnimation(n) {
    ...
    if (n) audioHandler.play("levelComplete");
    ...
    var recBounce = (timeout, count, n) => {
      if (n && count < 5) {
        var point = createRandomPointOnCircumference(
          [0, 0],
          500 - count * (400 / 10)
        );
        n.cx = point[0];
