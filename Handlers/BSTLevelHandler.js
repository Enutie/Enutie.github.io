    ...
    this.levels = [
      ...
      new Level(
        ...
        async function () {
          ...
          albot.say(['Right-click on the root to experiment with inserting, deleting, and finding values.",  "Click on a node to complete the level'], 0, 0)
      ...
      new Level(
        ...
        async function () {
          let albot_speed = (temp.skip_introduction ? 0 : undefined)
          await albot.say(['To delete a node, you first have to find it.', 'Then find the smallest value in its right subtree. That node will replace the node to be deleted.', 'To remove a subtree, click and drag over links to cut them.', 'You can add a node to the tree by dragging and dropping it on null pointers'], 40, 40, albot_speed)
          ...
          closure()
          if (!temp.skip_introduction) await timeout(1500)
          albot.say(['If you are unsure how to do this, press ESC and try the BST sandbox.', 'Try to delete ' + temp.successCircle.value], 0, 0)
